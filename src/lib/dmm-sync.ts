// DMM(FANZA)アフィリエイトAPI(公式API)から作品データを取得しSupabaseへ保存する共通ロジック。
// scripts/sync-dmm.ts（手動CLI実行）と src/app/api/cron/sync-dmm/route.ts（Vercel Cronからの自動実行）
// の両方から使う。スクレイピングは一切行わない（docs/architecture.md「出演者情報の扱い方針」の原則を
// カタログ取得にも適用）。出演者はDMM側が公式に返す iteminfo.actress のみを「公式クレジット」として
// 保存し、それ以外の出典による補完は行わない。
...
export interface DmmSyncSummary {
  fetched: number;
  upserted: number;
  skipped: number;
  errored: number;
...
export async function runDmmSync(options: {
  apiId: string;
  affiliateId: string;
  supabaseUrl: string;
  ...
  keyword: string | null;
  totalWanted: number;
}): Promise<DmmSyncSummary> {
  const supabase = createClient(options.supabaseUrl, options.supabaseServiceRoleKey, {
    auth: { persistSession: false },
  });

  const items = await fetchDmmItems(options.apiId, options.affiliateId, options.keyword, options.totalWanted);
